...
async function seedQuestions() {
    ...
    await question_1.QuestionModel.deleteMany({});
    // notun question gulo insert korbo
    const inserted = await question_1.QuestionModel.insertMany(questions);
    console.log(`Successfully inserted ${inserted.length} questions!`);
}
async function main() {
    try {
        await mongoose_1.default.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/yourdb');
